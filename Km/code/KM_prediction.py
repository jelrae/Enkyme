...
def KM_predicton(substrate_list, enzyme_list, KM, EC):
    ...
    df_KM = pd.DataFrame(data = {"substrate" : substrate_list, "enzyme" : enzyme_list, "value": KM, "EC" : EC, "index" : list(range(len(substrate_list)))})
    df_KM = merging_metabolite_and_enzyme_df(df_met, df_enzyme, df_KM)
    pickle.dump(df_KM, open(join(CURRENT_DIR, "../data", "transformed_data.pkl"), "wb"))
#     df_KM = pickle.load(open(join(CURRENT_DIR, "../data", "transformed_data.pkl"), "rb"))
    ...
    return(df_KM_valid, df_KM_invalid)
