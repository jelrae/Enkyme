...
def calculate_and_save_input_matrixes(dict, sample_ID, df, save_folder = join("../../data/GNN_input_data")):
    ind = int(sample_ID.split("_")[1])
    
    molecule_ID = df["Main Substrate"][ind]
    if molecule_ID[0] != 'C':
...
def calculate_atom_and_bond_feature_vectors(mol, mol_file):
    #check if feature vectors have already been calculated:
    try:
        os.mkdir("../../data/mol_feature_vectors")
    except FileExistsError:
        None
    #existing feature vector files:
    feature_files = os.listdir("../../data/mol_feature_vectors")
...
def calculate_atom_feature_vector_for_mol_file(mol, mol_file):
    ...
    with open("../../data/mol_feature_vectors/" + mol_file + "-atoms.txt", "wb") as fp:   #Pickling
...
def calculate_bond_feature_vector_for_mol_file(mol, mol_file):
    ...
    with open("../../data/mol_feature_vectors/" + mol_file + "-bonds.txt", "wb") as fp:   #Pickling
...
def create_bond_feature_matrix(mol_name, N =70):
    ...
    try:
        with open("../../data/mol_feature_vectors/" + mol_name + "-bonds.txt", "rb") as fp:   # Unpickling
...
def create_atom_feature_matrix(mol_name, N =70):
    try:
        with open("../../data/mol_feature_vectors/" + mol_name + "-atoms.txt", "rb") as fp:   # Unpickling
...
def download_mol_files():
    ...
    for i in range(0,22500):
        ...
        #check if it's empty
        if not r.content == b'':
            f= open("../../data/mol-files/" +kegg_id + ".mol","wb")
            f.write(r.content)
            f.close()
