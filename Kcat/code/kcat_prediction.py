...
def kcat_predicton(substrates, products, enzymes, values, RID, SID, EC):
    ...
    df_kcat = pd.DataFrame(data = {"substrates" : substrates, "products" : products,
    							 "enzyme" : enzymes, "value" : values, "Reaction ID": RID, "Sequence ID" : SID, "EC" : EC,  "index" : list(range(len(substrates)))})

    df_kcat = merging_reaction_and_enzyme_df(df_reaction, df_enzyme, df_kcat)
    df_kcat.to_pickle(join(CURRENT_DIR, "data", "transformed_data.pkl"))
    # df_kcat = pickle.load(open(join(CURRENT_DIR, "data", "transformed_data.pkl"), 'rb'))
